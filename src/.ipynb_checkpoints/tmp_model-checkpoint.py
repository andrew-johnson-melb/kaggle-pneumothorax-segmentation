...
class UNet(nn.Module):
    ...
    def __init__(self, layers_to_hook=None, encoder_model='resnet34', pretrained=True, freeze_encoder=True):
        ...
        self.encoder = get_vision_model(model_name=encoder_model, pretrained=pretrained, freeze_params=freeze_encoder)
        ...
        self.block_5_conv1 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = self.up_block_5(x)
        x = self.relu(self.block_5_conv1(x))
        x = self.block_5_conv2(x)
        # Let remove the dimension of size 1, since this in a binary classification problem
        # the channel dim should be 1. 
        return x
...
class DecoderBlock(nn.Module):
    """ Building block for the decoder of the Unet.
    ...
    """
    # TODO look at where conv tran parameters came from
    def __init__(self, in_channels, middle_channels, out_channels):
